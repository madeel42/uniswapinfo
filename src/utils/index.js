...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://app.kwikswap.org/swap?inputCurrency=${token0Address}`
  } else {
    return `https://app.kwikswap.org/swap?inputCurrency=${
      token0Address === '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2' ? 'ETH' : token0Address
...
export function getMiningPoolLink(token0Address) {
  return `https://app.kwikswap.org/#/kwik/ETH/${token0Address}`
}

export function getKwikswapAppLink(linkVariable) {
  let baseKwikswapUrl = 'https://app.kwikswap.org/#/kwik'
  if (!linkVariable) {
    return baseKwikswapUrl
  }

  return `${baseKwikswapUrl}/ETH/${linkVariable}`
}
