...
const UPDATE_ALL_PAIRS_IN_KWIKSWAP = 'UPDAUPDATE_ALL_PAIRS_IN_KWIKSWAPTE_TOP_PAIRS'
const UPDATE_ALL_TOKENS_IN_KWIKSWAP = 'UPDATE_ALL_TOKENS_IN_KWIKSWAP'
...
function reducer(state, { type, payload }) {
  switch (type) {
    ...
    case UPDATE_ALL_PAIRS_IN_KWIKSWAP: {
    ...
    case UPDATE_ALL_TOKENS_IN_KWIKSWAP: {
...
export default function Provider({ children }) {
  ...
  const updateAllPairsInKwikswap = useCallback((allPairs) => {
    dispatch({
      type: UPDATE_ALL_PAIRS_IN_KWIKSWAP,
      payload: {
        allPairs,
      },
    })
  ...
  const updateAllTokensInKwikswap = useCallback((allTokens) => {
    dispatch({
      type: UPDATE_ALL_TOKENS_IN_KWIKSWAP,
      payload: {
        allTokens,
      },
    })
      ...
      value={useMemo(
        () => [
          state,
          {
            ...
            updateEthPrice,
            updateTopLps,
            updateAllPairsInKwikswap,
            updateAllTokensInKwikswap,
          },
        ],
        [
          state,
...
async function getGlobalData(ethPrice, oldEthPrice) {
  ...
  try {
    ...
    data = result.data.kwikswapFactories[0]
    ...
    oneDayData = oneDayResult.data.kwikswapFactories[0]

    let twoDayResult = await client.query({
      query: GLOBAL_DATA(twoDayBlock?.number),
      fetchPolicy: 'cache-first',
    })
    twoDayData = twoDayResult.data.kwikswapFactories[0]

    let oneWeekResult = await client.query({
      query: GLOBAL_DATA(oneWeekBlock?.number),
      fetchPolicy: 'cache-first',
    })
    const oneWeekData = oneWeekResult.data.kwikswapFactories[0]

    let twoWeekResult = await client.query({
      query: GLOBAL_DATA(twoWeekBlock?.number),
      fetchPolicy: 'cache-first',
    })
    const twoWeekData = twoWeekResult.data.kwikswapFactories[0]
...
const getChartData = async (oldestDateToFetch) => {
  ...
  try {
    while (!allFound) {
      ...
      skip += 1000
      data = data.concat(result.data.kwikswapDayDatas)
      if (result.data.kwikswapDayDatas.length < 1000) {
        allFound = true
      }
    }
...
/**
 * Loop through every pair on kwikswap, used for search
 */
async function getAllPairsOnKwikswap() {
  try {
    let allFound = false
    let pairs = []
    let skipCount = 0
...
/**
 * Loop through every token on kwikswap, used for search
 */
async function getAllTokensOnKwikswap() {
  try {
    let allFound = false
    let skipCount = 0
    let tokens = []
...
export function useGlobalData() {
  const [state, { update, updateAllPairsInKwikswap, updateAllTokensInKwikswap }] = useGlobalDataContext()
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      let allPairs = await getAllPairsOnKwikswap()
      updateAllPairsInKwikswap(allPairs)

      let allTokens = await getAllTokensOnKwikswap()
      updateAllTokensInKwikswap(allTokens)
    }
    if (!data && ethPrice && oldEthPrice) {
      fetchData()
    }
  }, [ethPrice, oldEthPrice, update, data, updateAllPairsInKwikswap, updateAllTokensInKwikswap])
...
export function useAllPairsInKwikswap() {
...
export function useAllTokensInKwikswap() {
  const [state] = useGlobalDataContext()
