...
function init() {
  ...
  try {
    const parsed = JSON.parse(window.localStorage.getItem(KWIKSWAP))
...
export function Updater() {
  ...
  useEffect(() => {
    window.localStorage.setItem(KWIKSWAP, JSON.stringify({ ...state, [LAST_SAVED]: Math.floor(Date.now() / 1000) }))
  })
